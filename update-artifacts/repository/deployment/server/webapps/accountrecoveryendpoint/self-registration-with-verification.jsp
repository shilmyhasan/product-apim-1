...
<%
    ...
    try {
        if (!MultitenantUtils.isEmailUserName() && FrameworkUtils.retainEmailDomainOnProvisioning()) {
            User userWithTenantDomain = new User();
            userWithTenantDomain.setUsername(user.getUsername()
                + IdentityManagementEndpointConstants.TENANT_DOMAIN_SEPARATOR + user.getTenantDomain());
            userWithTenantDomain.setTenantDomain(user.getTenantDomain());
            userWithTenantDomain.setRealm(user.getRealm());
            userNameValidityStatusCode = selfRegistrationMgtClient
                    .checkUsernameValidity(userWithTenantDomain, skipSignUpEnableCheck);
        } else {
            userNameValidityStatusCode = selfRegistrationMgtClient
