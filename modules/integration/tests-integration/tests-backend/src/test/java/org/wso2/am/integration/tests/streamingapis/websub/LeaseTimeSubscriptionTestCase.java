...
public class LeaseTimeSubscriptionTestCase extends APIMIntegrationBaseTest {
    ...
    public void testInvokeWebSubApi() throws Exception {
        ...
        handleCallbackSubscription(SUBSCRIBE, apiEndpoint, callbackUrl, DEFAULT_TOPIC, topicSecret, "15",
                accessToken);
        ...
        Assert.assertEquals(received, 2); // no. of events received = no. of events sent + 1 subscribe event

        Thread.sleep(10000);
        webhookSender.send();

        sent = webhookSender.getWebhooksSent();
        received = callbackServerServlet.getCallbacksReceived();
        Assert.assertEquals(sent, 2);
        Assert.assertEquals(received, 2); // no. of events received = no. of events sent + 1 subscribe event

        handleCallbackSubscription(UNSUBSCRIBE, apiEndpoint, callbackUrl, DEFAULT_TOPIC, topicSecret, "15",
                accessToken);
